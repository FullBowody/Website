<template>
    <div class="flex w-fit max-w-full min-w-0">
        <div
            class="flex items-center justify-center w-fit h-fit rounded-md bg-transparent border-2 border-slate-100 dark:border-slate-600 transition-all max-w-full"
            :class="disabled
                ? 'text-slate-400 dark:text-slate-400 cursor-default'
                : (color !== 'slate'? 'text-'+color+'-500 dark:text-'+color+'-500' : 'text-slate-700 dark:text-slate-200') + ' hover:border-slate-200 hover:dark:border-slate-500 hover:text-slate-700 hover:dark:text-white'"
        >
            <component
